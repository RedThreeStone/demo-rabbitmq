 ...
 * @desciption 发布订阅模式 消费者发布一条消息到交换机,所有与这个交换机绑定的队列会收到全量的消息
 */
public class PubSub {
    ...
    private final String QUEUENAME1="pubqueue1";
    private final String QUEUENAME2="pubqueue2";
    private final String EXECHANGENAME="pubsubExchange";
...
//            propties.put("alternate-exchange","备用交换机名");
            channel.exchangeDeclare(EXECHANGENAME, BuiltinExchangeType.FANOUT,true,false,false,null);
            //如果不指定交换机 有个默认的交换机会被使用 routingkey 名为queue的名字,且不需要显示绑定
            channel.queueDeclare(QUEUENAME1,true,false,false,null);
            channel.queueDeclare(QUEUENAME2,true,false,false,null);
            channel.queueBind(QUEUENAME1,EXECHANGENAME,ROUTINGKEY);
            channel.queueBind(QUEUENAME2,EXECHANGENAME,ROUTINGKEY);
        } catch (IOException e) {
            e.printStackTrace();
